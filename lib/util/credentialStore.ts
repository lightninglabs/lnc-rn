...
const STORAGE_KEY = 'lnc-mobile';

/**
 * A wrapper around `AsyncStorage` used to store sensitive data required
...
export default class LncCredentialStore implements CredentialStore {
    // the data to store with AsyncStorage
    private persisted = {
        salt: '',
        cipher: '',
        serverHost: '',
    ...
    /** The namespace to use in the AsyncStorage key */
    ...
    constructor(namespace?: string, password?: string) {
        ...
        // load data stored in AsyncStorage
        this._load();
    }
    ...
    /** Saves persisted data to AsyncStorage */
    private async _save() {
        const key = `${STORAGE_KEY}:${this.namespace}`;
        await AsyncStorage.setItem(key, JSON.stringify(this.persisted));
    }
